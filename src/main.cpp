...
#include "MCModelOptions.cpp"
#include <iostream>

int main(int argc, char *argv[])
{
    std::shared_ptr<BSModelOptions> bs_model_options_ptr = std::make_shared<BSModelOptions>();
    std::shared_ptr<MCModelOptions> mc_model_options_ptr = std::make_shared<MCModelOptions>();
    ...
    const auto mc_call_contract = mc_model_options_ptr->CallMonteCarlo(S, X, r, sigma, t);
    const auto mc_put_contract = mc_model_options_ptr->PutMonteCarlo(S, X, r, sigma, t);
    ...
    std::cout << "European Call Option Price (MC): " << mc_call_contract.premium_ << std::endl;
    std::cout << "European Put Option Price (MC): " << mc_put_contract.premium_ << std::endl;
}